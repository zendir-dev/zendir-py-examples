...
from nominalpy import printer, types, Object, Simulation, System
...
spacecraft.invoke("SetGeodeticElements", 55.0, -5.0, 500000, "earth")
...
tracking_mount: Object = ground_station.add_child(
    "TrackingMount",
    In_AccessMsg=access_msg
)

# Create a CCD sensor, with some standard parameters.
ccd: Object = tracking_mount.add_child(
    "ChargeCoupledDevice",
    ...
    Efficency=0.8,              # 80% efficient with photon collection
...
simulation.set_tracking_interval(interval=1.0)
simulation.track_object(access_msg)
simulation.track_object(ccd.get_message("Out_CCDDataMsg"))

# Tick the simulation over 10 minutes
simulation.tick_duration(step=1.0, time=600)
...
data_ccd = simulation.query_dataframe(ccd.get_message("Out_CCDDataMsg"))
data_access = simulation.query_dataframe(access_msg)
...
ax4_1.set_position([0.55, 0.15, 0.19, 0.3])  # Adjust position and size of the first image
ax4_2.set_position([0.78, 0.15, 0.19, 0.3])  # Adjust position and size of the second image
...
ax3.fill_between(data_access["Time"], 0, 90, where=data_access["IsAccessible"], color='green', alpha=0.3)
ax3.text(10, 80, 'Accessible', color='green')
ax3.grid(True)

# Determine the properties of the CCD sensor used for the images
resolution = ccd.get("Resolution")
vmin = 1  # Minimum value for logarithmic scale
vmax = ccd.get("MaximumADU")  # Maximum value for the dataset
...
ax4_1.imshow(image, cmap='viridis', interpolation='nearest', origin='upper', norm=LogNorm(vmin=vmin, vmax=vmax))
ax4_1.axis('off')
...
data = np.maximum(data, 1)
image = data.reshape(resolution, resolution)
ax4_2.imshow(image, cmap='viridis', interpolation='nearest', origin='upper', norm=LogNorm(vmin=vmin, vmax=vmax))
ax4_2.axis('off')
